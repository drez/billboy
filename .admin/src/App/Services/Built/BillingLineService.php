...
class BillingLineService
{
    ...
    */
    ...
    public function deleteOne()
    {
        ...
        $obj = BillingLineQuery::create()->findPk(json_decode($this->request['i']));
    ...
    public function saveUpdate()
    {
        ...
        if(!empty($data['i'])) {
            ...
            $e = $this->Form->setCreateDefaultsBillingLine($data);

            if ($e->validate() && !$extValidationErr) {
                $e->save();
                $this->request['i'] = json_encode($e->getPrimaryKey());
                $this->afterSave($e, $data, true, $error, $this->request, $messages);
    ...
    */
    ...
    {
        $this->BillingLine['request'] = $this->request;
        $this->BillingLine['parentId'] = $this->request['data']['ip'];

            if($this->request['data']['ip']){
                $relData['IdBilling'] = $this->request['data']['ip'];
