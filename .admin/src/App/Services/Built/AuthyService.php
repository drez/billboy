    ...
    */
    ...
    public function deleteOne()
    {
        ...
        $obj = AuthyQuery::create()->findPk(json_decode($this->request['i']));
            ...
            if($obj->countClientsRelatedByIdCreation()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Client'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countClientsRelatedByIdModification()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Client'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countBillingsRelatedByIdCreation()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Billing'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countBillingsRelatedByIdModification()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Billing'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countBillingLinesRelatedByIdCreation()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Entries'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countBillingLinesRelatedByIdModification()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Entries'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countPaymentLinesRelatedByIdCreation()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Payment entry'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countPaymentLinesRelatedByIdModification()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Payment entry'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            ...
            if($obj->countProjectsRelatedByIdModification()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Project'. ", '', true,'User'); die( $error['onReadyJs'] );
            }
            if($obj->countTimeLinesRelatedByIdCreation()){
