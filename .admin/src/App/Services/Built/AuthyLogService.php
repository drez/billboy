...
class AuthyLogService
{
    ...
    */
    ...
    public function deleteOne()
    {
        ...
        $obj = AuthyLogQuery::create()->findPk(json_decode($this->request['i']));
    ...
    public function saveUpdate()
    {
        ...
        if(!empty($data['i'])) {
            ...
            $e = $this->Form->setCreateDefaultsAuthyLog($data);
            ...
            if ($e->validate() && !$extValidationErr) {
                ...
                $data['IdAuthy'] = json_encode($e->getIdAuthy());
            }else{
                $PropelErrorHandler = new PropelErrorHandler($e, $this->request['data']['ui'],_('Form field'), $extValidationErr, $this->virtualClassName);
                $error = $PropelErrorHandler->getValidationErrors();
            }
    ...
    */
    ...
    {
        $this->AuthyLog['request'] = $this->request;
        $this->AuthyLog['parentId'] = $this->request['data']['ip'];

            if($this->request['data']['ip']){
                $relData['IdAuthy'] = $this->request['data']['ip'];
