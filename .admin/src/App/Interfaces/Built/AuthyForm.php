...
. $this->cCmoreColsHeader;
        ...
        $this->arrayIdAuthyGroupOptions = $this->selectBoxAuthy_IdAuthyGroup($this, $emptyVar, $data);
            ...
            .div(
                form(div(input('text', 'Username', $this->searchMs['Username'], '  placeholder="'._('Name').'"',''),'','class="ac-search-item"').div(selectboxCustomArray('IdAuthyGroup', $this->arrayIdAuthyGroupOptions, 'Primary group' , "v='ID_AUTHY_GROUP'  s='d' ", $this->searchMs['IdAuthyGroup'], '', false), '', ' class="ac-search-item "').$this->hookListSearchTop
                    .div(
...
";
        ...
        if(!isMobile()) {
            $jqueryDatePicker = "
                $(\".AuthyListForm [j='date']\").attr('type', 'input');
                $(\".AuthyListForm [j='date']\").each(function(){
...
$this->fields['Authy']['Expire']['html'] = stdFieldRow(_("Expiration"), input('date', 'Expire', $dataObj->getExpire(), "  j='date' autocomplete='off' placeholder='YYYY-MM-DD' size='10'  s='d' class=''"), 'Expire', "", $this->commentsExpire, $this->commentsExpire_css, '', ' ', 'no');
