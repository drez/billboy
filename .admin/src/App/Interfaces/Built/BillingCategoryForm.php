...
class BillingCategoryForm extends BillingCategory
{
    ...
    **/
    ...
    function __construct(Request|array|null $request, array $args)
    {
        ...
        $this->args = $args;
        $this->model_name = 'BillingCategory';
        $this->virtualClassName = 'BillingCategory';
                    ...
                    foreach($order as $col => $sens){
                        if($sens){
                            ...
                            $this->orderReadyJsOrder .="
                                $(\"#BillingCategoryListForm [th='sorted'][c='".$col."']\").attr('sens', '".strtolower($sens)."')
...
. $this->cCmoreColsHeader;
            ...
            case 'add':
            ###### ADD
                if($_SESSION[_AUTH_VAR]->hasRights('BillingCategory', 'a') && !$this->setReadOnly){
                
                                $this->listAddButton = htmlLink(
                                    _("Add new")
                                ,_SITE_URL.$this->virtualClassName."/edit/", "id='addBillingCategory' title='"._('Add')."' class='button-link-blue add-button'");
            }
  ...
  'IdModification' => '',
        ...
        // if Search params
        $this->searchMs = $this->setSearchVar($request['ms'] ?? '', 'BillingCategory/');
        ...
        if( $pmpoData->isEmpty() ) {
            ...
            **/
            ...
            if(!$this->setReadOnly && !$this->setListRemoveDelete){
                if($_SESSION[_AUTH_VAR]->hasRights('BillingCategory', 'd')){
                    $this->canDelete = htmlLink("<i class='ri-delete-bin-7-line'></i>", "Javascript:", "class='ac-delete-link' j='deleteBillingCategory' ");
                }
            }
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent,'BillingCategoryControlsList', "class='custom-controls'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='BillingCategoryTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list" ')
                .$this->hookListBottom
                .$bottomRow
            , 'BillingCategoryListForm');

          #no parent

    $editUi = (empty($IdParent)) ? 'tabsContain' : 'editDialog';

    $editEvent .= "$(\"#BillingCategoryListForm td[j='editBillingCategory']\").bindEdit({
                    modelName:'" . $this->virtualClassName . "',
                    destUi: '{$editUi}'
                });
                
    $(\"#BillingCategoryListForm [j='deleteBillingCategory']\").bindDelete({
        modelName:'" . $this->virtualClassName . "',
        ui:'".$uiTabsId."',
...
";
    ...
    */
    public function setCreateDefaultsBillingCategory($data)
    {

        unset($data['IdBillingCategory']);
        $e = new BillingCategory();
    ...
    *	Make sure default value are set before save
    */
    public function setUpdateDefaultsBillingCategory($data)
    {

        
        $e = BillingCategoryQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of BillingCategory
    ...
    */
    ...
    public function getEditForm($id, $uiTabsId = 'tabsContain', $data=[], $error=[], $jsElement='', $jsElementType='')
    {
        ...
        ";
            ...
            $this->SaveButtonJs = "
                $('#formBillingCategory #saveBillingCategory').unbind('click.saveBillingCategory');
                $('#formBillingCategory #saveBillingCategory').remove();";
        }

        if($_SESSION[_AUTH_VAR]->hasRights('BillingCategory', 'a') && !$this->setReadOnly) {
            $this->formAddButton = htmlLink(_("Add new"), 'Javascript:;' , "id='addBillingCategory' title='"._('Add')."' class='button-link-blue add-button'");
        ...
        if($id && !$data['reload']) {
            ...
            $dataObj = $q->filterByIdBillingCategory($id)->findOne();
            
        }
        
        if($dataObj == null){
            $this->BillingCategory['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new BillingCategory();
            $this->BillingCategory['isNew'] = 'yes';
            if(is_array($data ))
               $dataObj->fromArray(array_filter($data));
            
        }else{
                $this->BillingCategory['isNew'] = 'no';
        }
...
$this->fields['BillingCategory']['Name']['html'] = stdFieldRow(_("Name"), input('text', 'Name', htmlentities($dataObj->getName()), "   placeholder='".str_replace("'","&#39;",_('Name'))."' size='35'  v='NAME' s='d' class='req'  ")."", 'Name', "", $this->commentsName, $this->commentsName_css, '', ' ', 'no');
        ...
        if(!$this->setReadOnly){
            $this->formSaveBar = div(	div( input('button', 'saveBillingCategory', _('Save'),' class="button-link-blue can-save"')
                                .input('hidden', 'formChangedBillingCategory','', 'j="formChanged"')
                                .input('hidden', 'idPk', urlencode($id), "s='d'")
                            .input('hidden', 'IdBillingCategory', $dataObj->getIdBillingCategory(), " s='d' pk").input('hidden', 'IdGroupCreation', $dataObj->getIdGroupCreation(), " s='d' nodesc").input('hidden', 'IdCreation', $dataObj->getIdCreation(), " s='d' nodesc").input('hidden', 'IdModification', $dataObj->getIdModification(), " s='d' nodesc")
        ...
        .form(
            ...
            div(
                ...
                .
$this->fields['BillingCategory']['Name']['html']
                
                .$this->formSaveBar
                .$this->hookFormInnerBottom
            ,"divCntBillingCategory", "class='divStdform' CntTabs=1 ".$this->ccStdFormOptions)
        , "id='formBillingCategory' class='mainForm formContent' ")
        .$this->hookFormBottom;
        ...
        if($_SESSION['mem']['BillingCategory']['ixmemautocapp'] and $_GET['Autocapp'] == 1) {
            $Autocapp = $_SESSION['mem']['BillingCategory']['ixmemautocapp'];
            unset($_SESSION['mem']['BillingCategory']['ixmemautocapp']);
        }
        ...
        setTimeout(function() {
            ...
            $('#formBillingCategory .js-select-label').SelectBox();
        }, 400);
        ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj)
    {
        ...
        if($fields == 'all') {
            foreach($this->fields['BillingCategory'] as $field => $ar) {
                $this->fields['BillingCategory'][$field]['html'] = $this->fieldsRo['BillingCategory'][$field]['html'];
            }
