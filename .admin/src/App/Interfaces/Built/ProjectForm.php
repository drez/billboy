...
. $this->cCmoreColsHeader;
        ...
        $this->arrayIdClientOptions = $this->selectBoxProject_IdClient($this, $emptyVar, $data);
            ...
            .div(
                form(div(selectboxCustomArray('IdClient', $this->arrayIdClientOptions, 'Client' , "v='ID_CLIENT'  s='d' ", $this->searchMs['IdClient'], '', true), '', ' class="ac-search-item "').div(input('date', 'Date', $this->searchMs['Date'], '  j="date"  title="'._('Date').'" placeholder="'._('Date').'"',''),'','class="ac-search-item"').div(input('text', 'Title', $this->searchMs['Title'], '  title="'._('Title').'" placeholder="'._('Title').'"',''),'','class="ac-search-item"').$this->hookListSearchTop
                    .div(
...
$this->fields['Project']['Date']['html'] = stdFieldRow(_("Start date"), input('date', 'Date', $dataObj->getDate(), "  j='date' autocomplete='off' placeholder='YYYY-MM-DD' size='10'  s='d' class='' title='Start date'"), 'Date', "", $this->commentsDate, $this->commentsDate_css, '', ' ', 'no');
...
.'' . $actionRowHeader, " ln='BillingLine' class=''");
  ...
  'IdModification' => '',
        ...
        $this->queryObjTimeLine = $q;
        
        $pmpoData =$q->paginate($search['page'], $maxPerPage);
        $resultsCount = $pmpoData->getNbResults();
        
        //custom hook
        if (method_exists($this, 'beforeChildListTimeLine')){
            $this->beforeChildListTimeLine();
        }
...
.'' . $actionRowHeader, " ln='TimeLine' class=''");
        ...
        if( $pmpoData->isEmpty() ){
            ...
            foreach($pmpoData as $data){
                ...
                // custom hooks
                if (method_exists($this, 'beforeListTrTimeLine')){ 
                    $this->beforeListTrTimeLine($altValue, $data, $i, $param, $actionRow);
                }
                
                $tr .= $param['tr_before'].
                        tr(
                            (isset($this->hookListColumnsTimeLineFirst)?$this->hookListColumnsTimeLineFirst:'').
                ...
                td(span((($altValue['Total']) ? $altValue['Total'] : str_replace(',', '.', $data->getTotal())) ?? ''." "), "  i='" . json_encode($data->getPrimaryKey()) . "' c='Total' class='right' " . $param['Total']." j='editTimeLine'") . 
                            (isset($this->hookListColumnsTimeLine)?$this->hookListColumnsTimeLine:'').
                            $actionRow
                            .$param['tr_after']
