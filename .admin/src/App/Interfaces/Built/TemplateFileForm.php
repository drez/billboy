                    ...
                    foreach($order as $col => $sens){
                        if($sens){
                            ...
                            $this->orderReadyJsOrder .="
                                $(\"#TemplateFileListForm [th='sorted'][c='".$col."']\").attr('sens', '".strtolower($sens)."')
...
. $this->cCmoreColsHeader;
            ...
            ###### ADD
                if($_SESSION[_AUTH_VAR]->hasRights('TemplateFile', 'a') && !$this->setReadOnly){
                    ...
                    $this->listAddButton .=
                                    ...
                                    .div('', 'filelist')
                                    ,'upload-TemplateFile',' class="ac-left-action-buttons" ');
  ...
  'IdModification' => '',
        ...
        // if Search params
        $this->searchMs = $this->setSearchVar($request['ms'] ?? '', 'TemplateFile/');
        ...
        htmlLink("<i class='ri-delete-bin-7-line'></i>", "Javascript:", "class='ac-delete-link' j='deleteTemplateFile' ") . $this->listActionCell, " class='actionrow' ");
                ...
                td(span(\htmlentities((($altValue['File']) ? $altValue['File'] : $data->getFile()) ?? '')." "), "  i='" . json_encode($data->getPrimaryKey()) . "' c='File' class=''  j='editTemplateFile'") . $cCmoreCols.$actionCell
                , " 
        ...
        $return['html'] =
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='TemplateFileTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
    ...
    $editEvent .= "$(\"#TemplateFileListForm td[j='editTemplateFile']\").bindEdit({
                    modelName:'" . $this->virtualClassName . "',
                    destUi: '{$editUi}'
                });
                
    $(\"#TemplateFileListForm [j='deleteTemplateFile']\").bindDelete({
        modelName:'" . $this->virtualClassName . "',
        ui:'".$uiTabsId."',
...
$(function(){
    ...
    var uploader = new plupload.Uploader({
        ...
        init: {
            ...
            UploadComplete: function(up, files) {
                ...
                document.location = '"._SITE_URL."TemplateFile/list/';
    ...
    */
    public function setCreateDefaultsTemplateFile($data)
    {
        ...
        //integer not required
        $e->setFile( ($data['File'] == '' ) ? null : $data['File']);
    ...
    *	Make sure default value are set before save
    */
    public function setUpdateDefaultsTemplateFile($data)
    {

        
        $e = TemplateFileQuery::create()->findPk(json_decode($data['i']));
    ...
    */
    ...
    public function getEditForm($id, $uiTabsId = 'tabsContain', $data=[], $error=[], $jsElement='', $jsElementType='')
    {
        ...
        ";
            ...
            $this->SaveButtonJs = "
                $('#formTemplateFile #saveTemplateFile').unbind('click.saveTemplateFile');
                $('#formTemplateFile #saveTemplateFile').remove();";
        }
        ...
        if($id && !$data['reload']) {
            ...
            $dataObj = $q->filterByIdTemplateFile($id)->findOne();
...
$this->fields['TemplateFile']['Name']['html'] = stdFieldRow(_("Name"), input('text', 'Name', htmlentities($dataObj->getName()), "   placeholder='".str_replace("'","&#39;",_('Name'))."' size='35'  v='NAME' s='d' class=''  ")."", 'Name', "", $this->commentsName, $this->commentsName_css, '', ' ', 'no');
$this->fields['TemplateFile']['File']['html'] = stdFieldRow(_("File"), input('text', 'File', htmlentities($dataObj->getFile()), "   placeholder='".str_replace("'","&#39;",_('File'))."' size='69'  v='FILE' s='d' class=''  ")."", 'File', "", $this->commentsFile, $this->commentsFile_css, '', ' ', 'no');
        ...
        if(!$this->setReadOnly){
            $this->formSaveBar = div(	div( input('button', 'saveTemplateFile', _('Save'),' class="button-link-blue can-save"')
                                .input('hidden', 'formChangedTemplateFile','', 'j="formChanged"')
...
.$this->fields['TemplateFile']['File']['html']
                
                .$this->formSaveBar
                .$this->hookFormInnerBottom
            ,"divCntTemplateFile", "class='divStdform' CntTabs=1 ".$this->ccStdFormOptions)
        , "id='formTemplateFile' class='mainForm formContent' ")
        .$this->hookFormBottom;
        ...
        if($_SESSION['mem']['TemplateFile']['ixmemautocapp'] and $_GET['Autocapp'] == 1) {
            $Autocapp = $_SESSION['mem']['TemplateFile']['ixmemautocapp'];
            unset($_SESSION['mem']['TemplateFile']['ixmemautocapp']);
        }
        ...
        setTimeout(function() {
            ...
            $('#formTemplateFile .js-select-label').SelectBox();
        }, 400);
        ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj)	
    {
        ...
        $this->fieldsRo['TemplateFile']['File']['html'] = stdFieldRow(_("File"), div( $dataObj->getFile(), 'File_label' , "class='readonly' s='d'")
                .input('hidden', 'File', $dataObj->getFile(), "s='d'"), 'File', "", $this->commentsFile, $this->commentsFile_css, 'readonly', ' ', 'no');


        if($fields == 'all') {
            foreach($this->fields['TemplateFile'] as $field => $ar) {
